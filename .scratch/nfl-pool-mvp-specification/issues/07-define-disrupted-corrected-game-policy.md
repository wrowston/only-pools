...
- Pool Owners and Pool Admins can see incident and repair status but cannot request provider resynchronization or deterministic recalculation. Automatic recovery belongs to the system and exceptional repair belongs only to the Production Operator, as superseded by [Define scoring, standings, and recalculation guarantees](./09-define-scoring-standings-recalculation.md). No Pool role or Production Operator can declare a winner or cancellation, enter or edit an NFL score, accept a provisional final, reopen a Pick Lock, suppress a Corrected Result, or otherwise override authoritative NFL data.
