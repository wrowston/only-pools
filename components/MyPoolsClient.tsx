...
function MyPoolsHome({ includeArchived }: { includeArchived: boolean }) {
  ...
  const liveMyPools = useQuery(
    ...
    isAuthenticated ? myPoolsQueryArgs(includeArchived) : "skip",
  );
...
/**
 * My Pools client surface. `includeArchived` comes from the server page so
 * soft-nav never suspends on the search-params client hook (which painted an
 * invisible skeleton and looked like an empty screen).
 */
export function MyPoolsClient({
  includeArchived,
}: {
  ...
  return <MyPoolsHome includeArchived={includeArchived} />;
}
